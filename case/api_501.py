...
class RunTest(TestCase, Tools, ddi_api, system, dnspage, resource):
    ...
    def test_add_auth_record_domain_validation_check(self):
        ...
        print('Step3: 用POST方法调用接口https://192.168.6.62:19393/api/dns/records，指定的域存在')
        payload3 = {
                    "domain": domain1,
                    "host": "f1",
                    "result": {"data": "1.1.1.1"}
        ...
        flag = self.authorization_find_specific_record(nodename=self.nodename[0],record="f1." + domain1,type='A',res='1.1.1.1')
        if not flag:
            self.exception.add('Step4: 页面未发现指定记录')


        print('Step5: 用POST方法调用接口https://192.168.6.62:19393/api/dns/records，指定的域不存在')
        payload5 = {
                    "domain": "abc.com",
                    "host": "f2",
                    "result": {"data": "1.1.1.1"}
        ...
        print('Step6: 用POST方法调用接口https://192.168.6.62:19393/api/dns/records，指定的域为空')
        payload6 = {
            "domain": "",
            "host": "f3",
            "result": {"data": "1.1.1.1"}
        ...
        if self.api_error is None:
            ...
            if not (status_code6 == 400 and res6['code'] == "InvalidDomain"):
                self.exception.add('Step6: ' + str(res6))
        else:
            self.exception.add('Step6: ' + str(self.api_error))

        print('Step7: 用POST方法调用接口https://192.168.6.62:19393/api/dns/records，未指定域')
        payload7 = {
            "host": "f4",
            "result": {"data": "1.1.1.1"}
        }
        ...
        if self.api_error is None:
            ...
            if not (status_code7 == 400 and res7['code'] == "InvalidDomain"):
