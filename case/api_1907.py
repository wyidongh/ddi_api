...
class RunTest(TestCase, Tools, system, ipam, ddi_api, resource):
    ...
    def test_ipam_get_addr_status_validation_check(self):
        ...
        print('Step2: 在postman上面 GET https://192.168.16.109:19393/api/ipam/ipstatus/name?ipSegment=192.168.1.0/24&ip=192.168.1.106,获取指定的ip地址状态')

        payload2 = {"ipSegment":"192.168.1.0/24",
                    "ip":"192.168.1.106"}
        r2 = self.get(url=url, params=payload2, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code2, res2 = r2
            if not (status_code2 == 400 and res2['code'] == "IPSegmentNotExist"):
                self.exception.add("Step2: " + str(r2))
        else:
            self.exception.add('Step2: ' + str(self.api_error))

        print('Step3: 在postman上面 GET https://192.168.16.109:19393/api/ipam/ipstatus/name?ipSegment=192.168.257.0/24&ip=192.168.257.11,获取指定的ip地址状态')

        payload3 = {"ipSegment":"192.168.257.0/24",
                    "ip":"192.168.257.11"}
        r3 = self.get(url=url, params=payload3, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code3, res3 = r3
            if not (status_code3 == 400 and res3['code'] == "InvalidIPSegment"):
                self.exception.add("Step3: " + str(r3))
        else:
            self.exception.add('Step3: ' + str(self.api_error))


        print('Step4: 在postman上面 GET https://192.168.16.109:19393/api/ipam/ipstatus/name?ipSegment=192.168.16.0/24&ip=192.168.16.266,获取指定的ip地址状态')
        ...
        print('Step5: 在postman上面 GET https://192.168.16.109:19393/api/ipam/ipstatus/name?ipSegment=192.168.16.0/24&ip=192.168.15.66,获取指定的ip地址状态')
