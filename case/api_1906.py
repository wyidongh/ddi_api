...
class RunTest(TestCase, Tools, system, ipam, ddi_api, resource):
    ...
    def test_ipam_get_addr_status_ip_list(self):
        ...
        print('Step2: 在postman上面 GET https://192.168.16.109:19393/api/ipam/ipstatus?ipSegment=192.168.16.0/24,获取地址状态ip列表')
        url = "https://" + self.serverip[0] + ":19393/api/ipam/ipstatus"
        payload2 = {"ipSegment":ipsegment}
        r2 = self.get(url=url, params=payload2, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code2, res2 = r2
            if not (status_code2 == 200 and res2['code'] == "Success" and len(res2['data']) == 100):
