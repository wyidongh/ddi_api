...
class RunTest(TestCase, Tools, resource, dhcp, ipam, ddi_api,system):
    ...
    def test_add_dhcp_subnet_version_validation_check(self):

        url = "https://" + self.serverip[0] + ":19393/api/dhcp/subnet"

        print('Step1: 用POST方法调用接口https://192.168.6.82:19393/api/dhcp/subnet，添加nodeName为“dhcp1”，ip为正常的8段的IPV6地址的策略，version不合法')
        ip1 = "2001::"
        mask1 = 64
        #gateway1 = "2001::1"
        dns1 = "2001::2"
        ...
        print('Step2: 用POST方法调用接口https://192.168.6.82:19393/api/dhcp/subnet，添加nodeName为“dhcp1”，ip为正常的ipv4地址的策略，version不合法')
        ip2 = "192.168.2.0"
        mask2 = 24
        ...
        print('Step3: 用POST方法调用接口https://192.168.6.82:19393/api/dhcp/subnet，添加nodeName为“dhcp1”，ip为正常的ipv4地址的策略，version不合法')
        ip3 = "192.168.3.0"
        mask3 = 24
        ...
        print('Step4: 用POST方法调用接口https://192.168.6.82:19393/api/dhcp/subnet，添加nodeName为“dhcp1”，ip为正常的ipv4地址的策略，version为空')
        ip4 = "192.168.4.0"
        mask4 = 24
