...
class RunTest(TestCase, Tools, ddi_api, system, dnspage, resource):
    ...
    def test_query_sepcific_doamin(self):
        ...
        self.authorization_add_ptr_record(nodename=self.nodename[0],domain=reverse_domain + ".in-addr.arpa",host='4',res="4.test.com")
        self.authorization_add_srv_record(nodename=self.nodename[0],domain=domain1,host='srv1',target="dev.test.com")
        ...
        if self.api_error is None:
            ...
            if status_code5 == 200 and res5['code'] == "Success" and len(res5['data']) == 14:
        ...
        if self.api_error is None:
            ...
            if status_code8 == 200 and res8['code'] == "Success" and len(res8['data']) == 8:
        ...
        if self.api_error is None:
            ...
            if not (status_code9 == 200 and res9['code'] == "Success" and len(res9['data']) == 2):
        ...
        print('Step14: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=A，查询A类型的记录，查看返回结果')
        payload14 = {
            "type": "A"
        }
        ...
        print('Step15: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=AAAA，查询AAAA类型的记录，查看返回结果')
        payload15 = {
            "type": "AAAA"
        }
        ...
        if self.api_error is None:
            ...
            if status_code15 == 200 and res15['code'] == "Success" and len(res15['data']) == 2:
                for zone in res15['data']:
                    type15 = zone.get('type')
                    if type15 != 'AAAA':
        ...
        print('Step16: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=CNAME，查询CNAME类型的记录，查看返回结果')
        payload16 = {
            "type": "CNAME"
        }
        ...
        if self.api_error is None:
            ...
            if status_code16 == 200 and res16['code'] == "Success" and len(res16['data']) == 2:
                for zone in res16['data']:
                    type16 = zone.get('type')
                    if type16 != 'CNAME':
        ...
        print('Step17: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=NS，查询NS类型的记录，查看返回结果')
        payload17 = {
            "type": "NS"
        }
        ...
        if self.api_error is None:
            ...
            if status_code17 == 200 and res17['code'] == "Success" and len(res17['data']) == 3:
                for zone in res17['data']:
                    type17 = zone.get('type')
                    if type17 != 'NS':
        ...
        print('Step18: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=MX，查询MX类型的记录，查看返回结果')
        payload18 = {
            "type": "MX"
        }
        ...
        print('Step19: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=PTR，查询PTR类型的记录，查看返回结果')
        payload19 = {
            "type": "PTR"
        }
        ...
        if self.api_error is None:
            ...
            if status_code19 == 200 and res19['code'] == "Success" and len(res19['data']) == 1:
                for zone in res19['data']:
                    type19 = zone.get('type')
                    if type19 != 'PTR':
        ...
        print('Step20: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=SRV')
        payload20 = {
            "type": "SRV"
        }
        r20 = self.get(url=url1, params=payload20, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code20, res20 = r20
            if not (status_code20 == 400 and res20['code'] == "InvalidResourceType"):
                self.exception.add('Step20: ' + str(res20))
        else:
            self.exception.add('Step20: ' + str(self.api_error))

        print('Step21: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？type=AD，查询错误的类型记录，查看返回结果')
        payload21 = {
            "type": "AD"
        }
        r21 = self.get(url=url1, params=payload21, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code21, res21 = r21
            if not (status_code21 == 400 and res21['code'] == "InvalidResourceType"):
                self.exception.add('Step21: ' + str(res21))
        else:
            self.exception.add('Step21: ' + str(self.api_error))

        print('Step22: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？domain=test.com&type=PTR，正向域查询PTR类型的记录，查看返回结果')
        payload22 = {
            "domain": domain1,
            "type": "PTR"
        }
        ...
        if self.api_error is None:
            ...
            if not (status_code22 == 200 and res22['code'] == "Success" and len(res22['data']) == 0):
                self.exception.add('Step22: ' + str(res22))
        else:
            self.exception.add('Step22: ' + str(self.api_error))

        print('Step23: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？domain=3.2.1.in-addr.arpa&type=MX，反向域查询MX类型的记录，查看返回结果')
        payload23 = {
        ...
        if self.api_error is None:
            ...
            if not (status_code23 == 200 and res23['code'] == "Success" and len(res23['data']) == 0):
                self.exception.add('Step23: ' + str(res23))
        else:
            self.exception.add('Step23: ' + str(self.api_error))

        print('Step24: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？start=0，从第一页开始查询，查看返回结果')
        payload24 = {
            "start": "0"
        }
        r24 = self.get(url=url1, params=payload24, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code24, res24 = r24
            if not (status_code24 == 200 and res24['code'] == "Success" and len(res24['data']) == 14):
                self.exception.add('Step24: ' + str(res24))
        else:
            self.exception.add('Step24: ' + str(self.api_error))

        print('Step25: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？start=1，从不存在的页码开始查询，查看返回结果')
        payload25 = {
            "start": "100"
        }
        r25 = self.get(url=url1, params=payload25, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code25, res25 = r25
            if not (status_code25 == 200 and res25['code'] == "Success" and len(res25['data']) == 0):
                self.exception.add('Step25: ' + str(res25))
        else:
            self.exception.add('Step25: ' + str(self.api_error))

        print('Step26: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？length=10，指定每页显示的条数，查看返回结果')
        payload26 = {
            "length": "10"
        }
        r26 = self.get(url=url1, params=payload26, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code26, res26 = r26
            if not (status_code26 == 200 and res26['code'] == "Success" and len(res26['data']) == 10):
                self.exception.add('Step26: ' + str(res26))
        else:
            self.exception.add('Step26: ' + str(self.api_error))

        print('Step27: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records？length=1000，指定的每页显示条数值不存在，查看返回结果')
        payload27 = {
            "length": "1000"
        }
        r27 = self.get(url=url1, params=payload27, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code27, res27 = r27
            if not (status_code27 == 200 and res27['code'] == "Success" and len(res27['data']) == 14):
                self.exception.add('Step27: ' + str(res27))
        else:
            self.exception.add('Step27: ' + str(self.api_error))

        print('Step28: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records?nodeName=node1&viewName=v1&domain=test.com&host=www&type=A&start=0&length=10，指定查询参数，查看返回结果')
        payload28 = {
            "nodeName": self.nodename[0],
            "viewName": view1,
            "domain": domain1,
        ...
        r28 = self.get(url=url1, params=payload28, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code28, res28 = r28
            if status_code28 == 200 and res28['code'] == "Success" and len(res28['data']) == 1:
                for zone in res28['data']:
                    nodename28 = zone.get('nodeName')
                    viewname28 = zone.get('viewName')
                    host28 = zone.get('host')
                    type28 = zone.get('type')
                    if not (nodename28 == self.nodename[0] and viewname28 == view1 and host28 == 'www.' + domain1 and type28 == "A"):
                        self.exception.add('Step28: ' + str(res28))
            else:
                self.exception.add('Step28: ' + str(res28))
        else:
            self.exception.add('Step28: ' + str(self.api_error))

        print('Step29: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records/5badea702a86c0a7516c042e，指定授权域的ID进行查询且该ID存在，查看返回结果')
        payload29 = {
            "viewName": view1
        }
        r29a = self.get(url=url1, params=payload29, auth=(self.api_user, self.api_password))
        id = ""
        if self.api_error is None:
            status_code29a, res29a = r29a
            if status_code29a == 200 and res29a["code"] == "Success" and len(res29a['data']) > 0:
                id = res29a['data'][0].get('id')
            else:
                self.exception.add("Step29a:" + str(res29a))
        else:
            self.exception.add("Step29a: " + self.api_error)
        url2 = url1 + "/" + str(id)
        r29b = self.get(url=url2, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code29b, res29b = r29b
            if not (status_code29b == 200 and res29b['code'] == "Success"):
                self.exception.add('Step29b ' + str(res29b))
        else:
            self.exception.add('Step29b ' + str(self.api_error))

        print('Step30: 用GET方法调用接口https://192.168.6.62:19393/api/dns/records/123456，指定授权域的ID进行查询且该ID不存在，查看返回结果')
        url3 = url1 + "/123456"
        r30 = self.get(url=url3, auth=(self.api_user, self.api_password))
        if self.api_error is None:
            status_code30, res30 = r30
            if not (status_code30 == 400 and res30['code'] == 'RecordNotExisted'):
                self.exception.add('Step30: ' + str(res30))
        else:
